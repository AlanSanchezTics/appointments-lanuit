...
vi.mock("@/lib/admin/appointments/service", () => ({
  confirmPendingAppointment: vi.fn(),
}));

const authMock = vi.mocked(auth);
const confirmPendingAppointmentMock = vi.mocked(confirmPendingAppointment);

describe("POST /api/admin/appointments/[appointmentId]/confirm", () => {
  ...
  it("returns 200 when the pending appointment is confirmed", async () => {
  ...
  it("returns 409 when the appointment is not pending anymore", async () => {
    authMock.mockResolvedValueOnce({ user: { name: "admin" } } as never);
    confirmPendingAppointmentMock.mockRejectedValueOnce(
