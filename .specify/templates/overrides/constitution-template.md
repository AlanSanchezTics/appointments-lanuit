...
### I. Docs First
`docs/specification.md` is the source of truth for product behavior.
Any feature spec, plan, or task artifact must be reconciled back to `docs/*` before the work is considered complete.

### II. Working Artifacts Are Temporary
`specs/*` is the working area for a single change.
Artifacts there support clarification, planning, and implementation, but they do not replace `docs/*`.
...
If behavior changes, the matching docs must be updated in the same change.
