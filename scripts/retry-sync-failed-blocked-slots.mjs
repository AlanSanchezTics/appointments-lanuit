...
async function createBlockedSlotCalendarEvent(calendarClient, input) {
  const isFullDayBlock = input.timeSlot === "00:00";
  const summary = isFullDayBlock
    ? `Bloqueo admin (${input.reason}) - día completo`
    : `Bloqueo admin (${input.reason}) - ${input.timeSlot}`;
  ...
  try {
    while (true) {
      ...
      for (const row of rows) {
        summary.scanned += 1;
        const lockName = `sync-retry:blocked-slot:${row.id}`;
        ...
        try {
          ...
          const googleEventId = row.googleEventId
            ?? await createBlockedSlotCalendarEvent(calendarClient, {
              date: toDateKey(row.date),
              timeSlot: toTimeKey(row.timeSlot),
              reason: row.reason,
            });

          const result = await prisma.blockedSlot.updateMany({
            where: {
              id: row.id,
              calendarSyncStatus: "SYNC_FAILED",
            },
